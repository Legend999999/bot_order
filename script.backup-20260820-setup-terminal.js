    ...
    en: {
        ...
        window.generateMatrixSignal = async function() {
            ...
            try {
                ...
                // Entry construction: Matrix quarter/floor/ceiling + TradeM zones when they are nearby.
                const channelSize = Math.max(stepVal*24, channel.high-channel.low);
                const entryHalf = Math.max(stepVal*1.25, channelSize*0.045);
                const stopBuffer = Math.max(stepVal*2.5, channelSize*0.10);

                let buyEntry = lines.floor;
                if(sup && Math.abs(sup._v-lines.floor) <= channelSize*0.75) buyEntry = +(0.65*lines.floor + 0.35*sup._v).toFixed(2);
                let sellEntry = lines.ceiling;
                if(res && Math.abs(res._v-lines.ceiling) <= channelSize*0.75) sellEntry = +(0.65*lines.ceiling + 0.35*res._v).toFixed(2);

                const buyLow = +(buyEntry-entryHalf).toFixed(2), buyHigh = +(buyEntry+entryHalf).toFixed(2);
                const sellLow = +(sellEntry-entryHalf).toFixed(2), sellHigh = +(sellEntry+entryHalf).toFixed(2);
                const buySL = +(buyLow-stopBuffer).toFixed(2);
                const sellSL = +(sellHigh+stopBuffer).toFixed(2);
                const buyRisk = Math.max(stepVal, buyEntry-buySL);
                const sellRisk = Math.max(stepVal, sellSL-sellEntry);
                const buyTP1 = +Math.max(lines.mid, buyEntry+buyRisk).toFixed(2);
                const buyTP2 = +Math.max(lines.ceiling, buyEntry+buyRisk*2).toFixed(2);
                const buyTP3 = +(buyEntry+buyRisk*3).toFixed(2);
                const sellTP1 = +Math.min(lines.mid, sellEntry-sellRisk).toFixed(2);
                const sellTP2 = +Math.min(lines.floor, sellEntry-sellRisk*2).toFixed(2);
                const sellTP3 = +(sellEntry-sellRisk*3).toFixed(2);

                const nearDistance = Math.max(stepVal*5, channelSize*0.20);
                const buyState = zoneState(currentPrice,buyLow,buyHigh,nearDistance);
                const sellState = zoneState(currentPrice,sellLow,sellHigh,nearDistance);
                const dBuy = Math.min(Math.abs(currentPrice-buyLow),Math.abs(currentPrice-buyHigh));
                const dSell = Math.min(Math.abs(currentPrice-sellLow),Math.abs(currentPrice-sellHigh));

                let primary='BOTH PLANS READY';
                if(tradeMBias==='bullish') primary='PRIMARY: BUY SIDE · TradeM bullish';
                else if(tradeMBias==='bearish') primary='PRIMARY: SELL SIDE · TradeM bearish';
                else primary = dBuy < dSell ? 'NEAREST: BUY ZONE' : dSell < dBuy ? 'NEAREST: SELL ZONE' : 'EQUAL DISTANCE';

                status.innerHTML = `
                    <div style="font-size:20px;color:var(--neon-gold);font-weight:900;">🎯 MATRIX 24 ENTRY MAP</div>
                    <div style="margin-top:7px;">Live XAU/USD: <b style="color:var(--neon-cyan);font-size:18px;">${currentPrice.toFixed(2)}</b></div>
                    <div style="margin-top:5px;color:#ddd;font-weight:900;">${primary}</div>
                    <div style="margin-top:5px;color:#999;">TradeM bias: ${tradeMBias.toUpperCase()} · Matrix ${trendDir.toUpperCase()} · Anchor ${base}</div>
                    <div style="margin-top:5px;color:${matrixGolden?'var(--neon-gold)':'#888'};">${matrixGolden?'⭐ IMPORTANT MATRIX HOUR':'Normal Matrix hour'} · Baghdad ${String(bp.hour).padStart(2,'0')}:${String(bp.minute).padStart(2,'0')}</div>
                    <div class="levels-grid">
                        ...
                        <div class="level-card"><small>Current</small><b style="color:var(--neon-cyan)">${currentPrice.toFixed(2)}</b></div>
                    </div>
                    ${sqLines.length?`<div style="margin-top:8px;color:#aaa;font-size:11px;">Square/time-price nearby: <b style="color:#fff">${sqLines.join(' · ')}</b></div>`:''}
                    <div style="margin-top:6px;color:#777;font-size:10px;">No candle feed required. These are rule-based Matrix/TradeM levels, not a guaranteed market direction.</div>`;

                buyBox.style.display='block';
                buyBox.innerHTML = `
                    <b style="font-size:18px;">🟢 BUY PLAN</b><br>
                    <b>Entry zone:</b> <span class="target-val">${buyLow} — ${buyHigh}</span><br>
                    <b>Best entry line:</b> <span class="target-val">${buyEntry.toFixed(2)}</span><br>
                    ${sup?`<b>Nearest TradeM support:</b> <span class="target-val">${sup._v.toFixed(2)}</span><br>`:''}
                    <b>Stop loss:</b> <span style="color:#ff5555;font-family:monospace;font-weight:900;">${buySL}</span><br>
                    <b>TP1:</b> <span class="target-val">${buyTP1}</span> · <b>TP2:</b> <span class="target-val">${buyTP2}</span> · <b>TP3:</b> <span class="target-val">${buyTP3}</span><br>
                    <b>Logic:</b> lower Matrix boundary / support reaction area.<br>
                    <div class="plan-state ${buyState.cls}">${buyState.label}</div>`;

                sellBox.style.display='block';
                sellBox.innerHTML = `
                    <b style="font-size:18px;">🔴 SELL PLAN</b><br>
                    <b>Entry zone:</b> <span class="target-val">${sellLow} — ${sellHigh}</span><br>
                    <b>Best entry line:</b> <span class="target-val">${sellEntry.toFixed(2)}</span><br>
                    ${res?`<b>Nearest TradeM resistance:</b> <span class="target-val">${res._v.toFixed(2)}</span><br>`:''}
                    <b>Stop loss:</b> <span style="color:#ff5555;font-family:monospace;font-weight:900;">${sellSL}</span><br>
                    <b>TP1:</b> <span class="target-val">${sellTP1}</span> · <b>TP2:</b> <span class="target-val">${sellTP2}</span> · <b>TP3:</b> <span class="target-val">${sellTP3}</span><br>
                    <b>Logic:</b> upper Matrix boundary / resistance reaction area.<br>
                    <div class="plan-state ${sellState.cls}">${sellState.label}</div>`;
            } catch(e) {
